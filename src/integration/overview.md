...
> 太乙智启的集成开发文档按 **业务层 → 应用层 → 协议层** 三个维度组织，由浅入深。本篇是总导读，帮助你选择合适的集成路径与阅读顺序。
...
| **taiyiflow-chatui** | 对话前端：Vue 3 + Quasar 的完整智能体客户端（Web SPA / 桌面端），也是 JS SDK 中 iframe 加载的页面 | 二次开发者、白标定制厂商 |
...
- **ChatUI 是完整的智能体客户端**：对话、客户端工具执行、技能管理、MCP、语音通话、子任务协作，Web 与桌面端双形态。
...
| [智能体前端集成与二次开发](/integration/chatui) | 部署对话前端、桌面端、白标定制扩展点 |
...
1. [智能体前端集成与二次开发](/integration/chatui) → 2. [认证与鉴权](/integration/authentication) → 3. [客户端工具协议](/integration/client-tool-protocol)

### 场景四：深度二次开发 / 贡献代码

阅读 [智能体前端集成与二次开发](/integration/chatui) 与协议层各篇，获取源码授权后配合仓库内文档深入。
