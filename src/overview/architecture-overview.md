...
- **模型无关**：通过 Provider 抽象层对接任意大模型，切换模型不改业务代码
...
| **管理后台** | React + Vite | 可视化流程设计器、用户/模型/知识库管理 | 静态资源，Nginx 托管 |
| **对话前端** | React + Vite | 员工日常对话入口（Web 版） | 静态资源，Nginx 托管 |
| **桌面客户端** | Electron / Tauri | 本地文件操作、浏览器自动化、语音交互 | 用户本机安装 |
...
| **模型 Provider** | 统一接口对接 OpenAI 兼容 / 国产大模型，支持多模型并行 |
...
太乙智启不绑定任何特定大模型厂商。所有模型调用通过统一的 Provider 接口：
...
| 管理前端 | React 18 + Vite + TailwindCSS + ReactFlow |
| 对话前端 | React 18 + Vite + TailwindCSS |
| 桌面客户端 | 跨平台桌面框架 |
| CLI | Go |
| 向量数据库 | Milvus（默认） |
| 关系数据库 | PostgreSQL |
